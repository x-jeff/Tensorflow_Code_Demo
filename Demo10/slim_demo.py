...
def _convert_dataset(split_name, filename, class_names_to_ids, dataset_dir):
    ...
    # 计算每个数据块有多少数据
    num_per_shard = int(len(filename) / _NUM_SHARDS)
                ...
                with tf.python_io.TFRecordWriter(output_filename) as tfrecord_writer:
                    ...
                    for i in range(start_ndx, end_ndx):
                        try:
                            ...
                            # 如果有错误的图像则跳过,不做其他处理
                            print("Could not read: ", filename[i])
                            print("Error: ", e)
                            print("Skip it\n")
    sys.stdout.write('\n')
    sys.stdout.flush()
...
if __name__ == '__main__':
    ...
    if _dataset_exists(DATASET_DIR):
        ...
        # 把分类转为字典格式，类似于{"cat":0,"dog":1,"flower":2}
        class_names_to_ids = dict(zip(class_names, range(len(class_names))))

        # 把数据切分为训练集和测试集
        random.seed(_RANDOM_SEED)
        random.shuffle(photo_filenames)  # 所有元素随机排序
